...
interface DbOperations {
	...
	getOrCreateUsage(
		userId: string,
		month: string,
	): Promise<{ id: string; transcriptionMinutes: number | null; cleanupRequests: number | null }>;
	incrementUsage(
		userId: string,
...
if (isPostgres) {
	...
	dbOps = {
		async createUser(email, password, name) {
			...
			await db.insert(users).values({ id, email, passwordHash, name: name || null });
		...
		async createOAuthUser(email, googleId, name) {
			...
			await db.insert(users).values({ id, email, googleId, name: name || null });
			const user = await this.getUserById(id);
			if (!user) throw new Error("Failed to create user");
			return user;
		},
		async getUserById(id) {
			...
			return result ? { ...result, createdAt: result.createdAt?.toISOString() ?? null } as User : null;
		},
		...
		async insertDictation(userId, id, rawText, cleanedText, provider, durationMs) {
			await db.insert(dictationHistory).values({ id, userId, rawText, cleanedText, provider, durationMs });
		},
		async updateDictationCleaned(id, cleanedText) {
			await db.update(dictationHistory).set({ cleanedText }).where(eq(dictationHistory.id, id));
		},
		async getHistory(userId) {
			const results = await db.select().from(dictationHistory)
				.where(eq(dictationHistory.userId, userId))
				.orderBy(desc(dictationHistory.createdAt));
			return results.map(r => ({ ...r, createdAt: r.createdAt?.toISOString() ?? null })) as DictationEntry[];
		},
		async updateDictationAudioUrl(id, audioUrl) {
			await db.update(dictationHistory).set({ audioUrl }).where(eq(dictationHistory.id, id));
		},
		async updateUserSubscription(userId, data) {
			await db.update(users).set(data).where(eq(users.id, userId));
		},
		async getUserByPolarCustomerId(customerId) {
			const [result] = await db.select().from(users).where(eq(users.polarCustomerId, customerId));
			return result ? { ...result, createdAt: result.createdAt?.toISOString() ?? null } as User : null;
		},
		async getOrCreateUsage(userId, month) {
			let [record] = await db.select().from(usageTracking)
			...
			if (!record) {
				...
				[record] = await db.select().from(usageTracking).where(eq(usageTracking.id, id));
			}
			return record;
		},
		async incrementUsage(userId, type, amount) {
			...
			const current = (type === "transcriptionMinutes" ? usage.transcriptionMinutes : usage.cleanupRequests) || 0;
			await db.update(usageTracking).set({
				[type]: current + amount,
			}).where(eq(usageTracking.id, usage.id));
		},
		async hasPolarEvent(eventId) {
			const [result] = await db.select().from(polarEvents).where(eq(polarEvents.id, eventId));
			return !!result;
		},
		...
		async searchHistory(userId, query) {
			const results = await db.select().from(dictationHistory)
				...
				.orderBy(desc(dictationHistory.createdAt));
			return results
				.filter(e => e.rawText.includes(query) || e.cleanedText.includes(query))
				.map(r => ({ ...r, createdAt: r.createdAt?.toISOString() ?? null })) as DictationEntry[];
		},
		async addToWaitlist(email, source) {
			const id = crypto.randomUUID();
			await db.insert(waitlist).values({ id, email, source: source || "website" });
			return { id, email, source: source || "website", createdAt: new Date().toISOString() };
		},
		async getWaitlistEntry(email) {
			const [result] = await db.select().from(waitlist).where(eq(waitlist.email, email));
			return result ? { ...result, createdAt: result.createdAt?.toISOString() ?? null } as WaitlistEntry : null;
		},
		...
		async getAllWaitlistEntries() {
			...
			return results.map(r => ({ ...r, createdAt: r.createdAt?.toISOString() ?? null })) as WaitlistEntry[];
		},
	};
} else {
	...
	dbOps = {
		async createUser(email, password, name) {
			const id = crypto.randomUUID();
			const passwordHash = await Bun.password.hash(password, "argon2id");
			db.insert(users).values({ id, email, passwordHash, name: name || null }).run();
			const user = await this.getUserById(id);
			if (!user) throw new Error("Failed to create user");
			return user;
		},
		async createOAuthUser(email, googleId, name) {
			...
			db.insert(users).values({ id, email, googleId, name: name || null }).run();
			const user = await this.getUserById(id);
			if (!user) throw new Error("Failed to create user");
			return user;
		},
		...
		async upsertProfile(userId, customWords, contextPrompt, writingStyle) {
			...
			if (existing) {
				db.update(profiles).set({
					...
					...(contextPrompt !== undefined && { contextPrompt }),
					...(writingStyle !== undefined && { writingStyle }),
				}).where(eq(profiles.userId, userId)).run();
			} else {
		...
		async insertDictation(userId, id, rawText, cleanedText, provider, durationMs) {
			db.insert(dictationHistory).values({ id, userId, rawText, cleanedText, provider, durationMs }).run();
		},
		async updateDictationCleaned(id, cleanedText) {
			db.update(dictationHistory).set({ cleanedText }).where(eq(dictationHistory.id, id)).run();
		},
		async getHistory(userId) {
			return db.select().from(dictationHistory)
				.where(eq(dictationHistory.userId, userId))
				.orderBy(desc(dictationHistory.createdAt))
				.all();
		},
		async updateDictationAudioUrl(id, audioUrl) {
			db.update(dictationHistory).set({ audioUrl }).where(eq(dictationHistory.id, id)).run();
		},
		async updateUserSubscription(userId, data) {
			db.update(users).set(data).where(eq(users.id, userId)).run();
		},
		async getUserByPolarCustomerId(customerId) {
			const result = db.select().from(users).where(eq(users.polarCustomerId, customerId)).get();
			return result || null;
		},
		async getOrCreateUsage(userId, month) {
			let record = db.select().from(usageTracking)
				.where(and(eq(usageTracking.userId, userId), eq(usageTracking.month, month)))
				.get();
			if (!record) {
				const id = crypto.randomUUID();
				db.insert(usageTracking).values({ id, userId, month }).run();
				record = db.select().from(usageTracking).where(eq(usageTracking.id, id)).get()!;
			}
			return record;
		},
		async incrementUsage(userId, type, amount) {
			...
			const usage = await this.getOrCreateUsage(userId, month);
			const current = (type === "transcriptionMinutes" ? usage.transcriptionMinutes : usage.cleanupRequests) || 0;
			db.update(usageTracking).set({
				[type]: current + amount,
			}).where(eq(usageTracking.id, usage.id)).run();
		},
		async hasPolarEvent(eventId) {
			const result = db.select().from(polarEvents).where(eq(polarEvents.id, eventId)).get();
			return !!result;
		},
		async insertPolarEvent(eventId, type, payload) {
		...
		async searchHistory(userId, query) {
			const results = db.select().from(dictationHistory)
				.where(eq(dictationHistory.userId, userId))
				.orderBy(desc(dictationHistory.createdAt))
				.all();
			return results.filter(e => e.rawText.includes(query) || e.cleanedText.includes(query));
		},
		async addToWaitlist(email, source) {
			const id = crypto.randomUUID();
			db.insert(waitlist).values({ id, email, source: source || "website" }).run();
			return { id, email, source: source || "website", createdAt: new Date().toISOString() };
		},
		async getWaitlistEntry(email) {
			const result = db.select().from(waitlist).where(eq(waitlist.email, email)).get();
			return result || null;
		},
		async getWaitlistCount() {
...
export const updateDictationAudioUrl = dbOps.updateDictationAudioUrl.bind(dbOps);
export const updateUserSubscription = dbOps.updateUserSubscription.bind(dbOps);
export const getUserByPolarCustomerId = dbOps.getUserByPolarCustomerId.bind(dbOps);

...
export function AuthProvider({ children }: { children: ReactNode }) {
	...
	useEffect(() => {
		const checkSetupMode = async () => {
			try {
				const mode = await invoke<string | null>("get_setting", {
					key: "setup_mode",
				});
				setSetupMode(mode);
			} catch {
	...
	const refreshUser = useCallback(async () => {
		...
		try {
			...
			// Update setupMode in state regardless of user existence
			setSetupMode(mode);
			...
			if (mode === "local") {
				try {
					...
					// No local user yet, that's ok - they're about to create one
					setUser(null);
				}
			} else {
				setUser(null);
	...
	const updateOnboarding = async (
		...
		if (user?.isLocal) {
			...
			const { user: apiUser } = await api.updateOnboarding(
				completed,
				newSetupMode,
			);
			setUser({
